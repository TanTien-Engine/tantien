...
{
    return clamp( vec3( abs( H * 6.0f - 3.0f ) - 1.0f,
                        2.0f - abs( H * 6.0f - 2.0f ),
                        2.0f - abs( H * 6.0f - 4.0f )), 0.0, 1.0);
}
...
vec3 KelvinToRGB( in float k )
{
    ...
    {
        ret.r        = 1.0f;
        ret.g        = clamp( 0.39008157876901960784f * log( kelvin ) - 0.63184144378862745098f, 0.0, 1.0 );
    }
    else
    {
        float t      = max( kelvin - 60.0f, 0.0f );
        ret.r        = clamp( 1.29293618606274509804f * pow( t, -0.1332047592f ), 0.0, 1.0);
        ret.g        = clamp( 1.12989086089529411765f * pow( t, -0.0755148492f ), 0.0, 1.0);
    }
    if( kelvin >= 66.0f )
        ret.b        = 1.0f;
        ...
        ret.b        = clamp( 0.54320678911019607843f * log( kelvin - 10.0f ) - 1.19625408914f, 0.0, 1.0 );
    return ret;
}
...
// Reference white D65
#define reference_white     vec3( 0.95047, 1.0, 1.08883 )
...
vec3 pd80_xyz_to_lab( vec3 c )
{
    // .xyz output contains .lab
    vec3 w       = max( c / reference_white, 0.0f );
    vec3 v;
...
vec3 pd80_lab_to_xyz( vec3 c )
{
    vec3 v;
    v.y            = ( c.x + 16.0 ) / 116.0;
    v.x            = c.y / 500.0 + v.y;
    v.z            = v.y - c.z / 200.0;
    return vec3(( v.x * v.x * v.x > E_val ) ? v.x * v.x * v.x : ( 116.0 * v.x - 16.0 ) / K_val,
                  ( c.x > K_val * E_val ) ? v.y * v.y * v.y : c.x / K_val,
                  ( v.z * v.z * v.z > E_val ) ? v.z * v.z * v.z : ( 116.0 * v.z - 16.0 ) / K_val ) *
                  reference_white;
}

vec3 pd80_srgb_to_xyz( vec3 c )
{
...
vec3 pd80_srgb_to_lab( vec3 c )
{
    vec3 lab = pd80_srgb_to_xyz( c );
    lab        = pd80_xyz_to_lab( lab );
    return lab / vec3( 100.0, 108.0, 108.0 );
}

vec3 pd80_lab_to_srgb( vec3 c )
{
    vec3 rgb = pd80_lab_to_xyz( c * vec3( 100.0, 108.0, 108.0 ));
